...
int timelineDisplay()
{
	...
	int twoPercent = (length / 44);
	int area = (selTimeline / twoPercent);
	std::string scrollBar;
	for (int i = 0; i < area; i++)
		scrollBar.append(" ");
	scrollBar.append("|");
	for (int i = 0; i < (42 - area); i++)
		scrollBar.append(" ");
	printf("\x1b[18;0H  " RESET "+-------------------------------------------+");
	printf("\x1b[19;0H  " RESET "|" COLOR BRIGHT CSEP WHITE CEND "%s" RESET "|", scrollBar.c_str());
	printf("\x1b[20;0H  " RESET "+-------------------------------------------+");
	return 0;
}
...
int timeline(int upperrv)
{
	...
	if (strcmp(returnvalue, "START") == 0)
		ireturnvalue = 2;
	if ((strcmp(returnvalue, "LEFT") == 0) || (strcmp(returnvalue, "RIGHT") == 0))
		ireturnvalue = 1;
